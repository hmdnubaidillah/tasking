...
import { useLogin } from "@/hooks/hook.user";
import { ErrorResponse, LoginFormType } from "@/types";
import { loginSchema } from "@/libs/lib.validation";
...
export default function LoginForm() {
  ...
  const { mutate, data, isPending, isSuccess, error } = useLogin();
  ...
  return (
    <div>
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-3">
        ...
        {isPending ? (
          <Button type="submit" disabled={true} className="text-lg">
            <i className="uil uil-spinner animate-spin text-2xl" />
          </Button>
        ) : (
          <Button type="submit" disabled={false} className="text-lg">
            Login
          </Button>
        )}
      </form>
